...
- **After** = result of `./mvnw clean test` at the head of this PR (31 tests, 0 failures, 0 errors).
...
| FR-MST-002 Multiple Service Transactions | Flag a user transacting in more than 3 distinct services within any 5-minute sliding window (window boundary inclusive). | `DISTINCT_SERVICE_THRESHOLD = 3` (exclusive) (`MultipleServiceTransactionImpl.java:26`), `DISTINCT_SERVICE_WINDOW_SECONDS = 5*60` (`MultipleServiceTransactionImpl.java:23`) | `TransactionEvent.serviceID`, `TransactionEvent.timestamp`, `userId` | `src/main/java/com/example/fraudapplication/service/impl/MultipleServiceTransactionImpl.java:55` (window eviction `:45-48`; pre-change: same file `:42`) |
| FR-PPA-003 Ping-Pong Activity | Flag four consecutive transactions alternating between exactly two services (A,B,A,B) whose first and last transactions are at most 10 minutes apart (inclusive). | `PING_PONG_SEQUENCE_LENGTH = 4` (`PingPongActivityServiceImpl.java:22`), `PING_PONG_WINDOW_SECONDS = 10*60` (`PingPongActivityServiceImpl.java:19`) | `TransactionEvent.serviceID`, `TransactionEvent.timestamp`, `userId` | `src/main/java/com/example/fraudapplication/service/impl/PingPongActivityServiceImpl.java:51` (pattern `:45-49`; pre-change: same file `:58-60`) |
...
| Empty transaction list | `emptyTransactionListIsHandled` `:110` | PASS | PASS |
...
| Case | Test (file:line) | Before | After |
| --- | --- | --- | --- |
| Just below window — bounce spanning 599s | `bounceJustInsideWindowAlertsOnce` `:47` | FAIL (expected 1 alert, got 0) | PASS |
| Exactly at window edge — bounce spanning 600s | `bounceExactlyAtWindowEdgeAlertsOnce` `:55` | FAIL (expected 1 alert, got 0) | PASS |
| Just outside window — bounce spanning 601s | `bounceJustOutsideWindowDoesNotAlert` `:63` | PASS | PASS |
| Negative case — A,B,C,B is not a bounce | `nonAlternatingSequenceDoesNotAlert` `:71` | PASS | PASS |
| Negative case — only 3 transactions (A,B,A) | `thirdBounceIsRequiredBeforeAlerting` `:79` | PASS | PASS |
| Adversarial — bounce padded so it lands on the final transactions of the stream | `bounceAtTheEndOfTheStreamIsStillDetected` `:86` | FAIL (expected 1 alert, got 0) | PASS |
| Adversarial — third-service decoy injected between bounces (documented residual risk) | `bounceSplitByAnInterleavedThirdServiceEvadesTheRule` `:96` | PASS | PASS |
| Out-of-order delivery evaluated chronologically | `outOfOrderEventsAreEvaluatedChronologically` `:106` | FAIL (expected 1 alert, got 0) | PASS |
| Empty transaction list | `emptyTransactionListIsHandled` `:114` | FAIL (`IndexOutOfBoundsException`) | PASS |
...
| FR-MST-002 | Window anchored on a mutable "first transaction" instead of sliding, and only the anchor's service was ever evicted, so services from long-expired windows kept accumulating (false positives) (`:26-41` pre-change) | True sliding window with eviction of every event older than 5 minutes (`:42-53`) |
| FR-MST-002 | Once breached, one alert was appended for *every* remaining transaction (`:42-44` pre-change) | One alert per breach, window reset afterwards (`:55-58`) |
| FR-MST-002 | `transactions.get(0)` on an empty list threw `IndexOutOfBoundsException`; unsorted input evaluated in arrival order (`:26` pre-change) | Empty/blank input guarded and events sorted chronologically (`:33-40`) |
| FR-PPA-003 | Detection loop ran `index+2 .. size-2`, so a bounce at the start or end of the stream was never examined (`:45` pre-change) | Every 4-transaction sliding sequence is examined (`:38-43`) |
| FR-PPA-003 | Two-service state map could retain stale services, alerting on non-alternating sequences (`:58-69` pre-change) | Explicit A,B,A,B pattern check on consecutive transactions (`:45-47`) |
| FR-PPA-003 | `transactions.get(0)` on an empty list threw `IndexOutOfBoundsException`; unsorted input evaluated in arrival order (`:27` pre-change) | Short/blank input guarded and events sorted chronologically (`:29-36`) |
...
| FR-PPA-003 | Interleaving: a decoy transaction in a third service breaks the consecutive A,B,A,B pattern. | `PingPongActivityServiceImplTest.bounceSplitByAnInterleavedThirdServiceEvadesTheRule:96` |
